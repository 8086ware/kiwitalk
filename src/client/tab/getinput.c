	...
	if(s_poll[0].revents & POLLIN) {
		...
		if(c.ctrl_character == TM_KEY_CR || c.ctrl_character == TM_KEY_LF) {
			...
			for(int i = 0; i < tab_amount; i++) { 
				tm_win_modify((*tabs)[i]->window, 0, 0, scr_cols, scr_rows, 1);
				tm_win_border((*tabs)[i]->window);
				tm_win_modify((*tabs)[i]->window_text, 1, 1, scr_cols - 2, scr_rows - 2, 0);
				tm_win_modify((*tabs)[i]->window_input, 1, scr_rows - 2, scr_cols - 2, 1, 0);
			}
		}
